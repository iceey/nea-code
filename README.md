...
The hover text is built by pairing up the accuracy and duration lists with `zip()` -- cleaner than looping with an index counter. Plotly's `hovertemplate` is fiddly though, the formatting syntax is different from Python f-strings, but once it works it's really nice for exploring the data interactively.
...
```
gradio_phase18/
├── app_ui.py          # Main application (589 lines, +20 from Phase 17)
├── utils.py           # Preprocessing function (34 lines)
├── models.py          # 3 architectures (unchanged)
├── init_db.py         # Database initialisation (71 lines)
...
## Differences from Phase 17

| Aspect | Phase 17 | Phase 18 |
|--------|----------|----------|
| **Architecture comparison** | Bar chart (averages) | Scatter plot (individual runs) |
| **Data shown** | Averaged accuracy per architecture | Each run as a separate point |
| **Colour coding** | Single colour | Per-architecture colours |
| **Hover info** | Basic | Custom hover with accuracy + duration |
| **app_ui.py** | 569 lines | 589 lines (+20) |
