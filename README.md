...
The jump from ~70 lines (Phase 7) to 176 lines feels big, but most of it is just setting up the buttons and layout.
...
The prediction tab loads the model once when the app starts. So if you train a new model, the prediction tab is still using the old one until you restart the app. I know about this but it's not worth fixing right now.

## File Structure

```
gradio_phase8/
├── app_ui.py          # Main application (176 lines)
├── models.py          # Model architectures (65 lines)
