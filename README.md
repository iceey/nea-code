...
**Requires Python 3.9–3.12** (TensorFlow 2.16.2 doesn't support 3.13+ yet). I developed on Python 3.9.6 (macOS system Python) but any version from 3.9 to 3.12 should work.
...
### Minimal Code Changes in Phase 23

Phase 23 is mostly about documentation. The only code changes were tidying up comments in models.py -- removed some inline comments from the model layer definitions to let the docstrings do the talking instead. No functional behaviour was changed. I wanted to keep it clear what was building vs what was writing about.
...
**Note**: These versions need Python 3.9–3.12. TensorFlow 2.16.2 doesn't support 3.13+ yet.
...
- **TensorFlow 2.16.2**: Neural network training and inference
...
- **macOS**: Developed on a Mac with Python 3.9.6, but 3.9–3.12 all work
...
## Python Concepts I Picked Up

Some of these I already knew from lessons and some I had to learn as I went:

- **`np.expand_dims` / `.reshape`**: The model expects a batch of images shaped `(n, 28, 28)` but a single image is just `(28, 28)`. `expand_dims` adds the extra dimension; later I used `.reshape(1, 28, 28)` which does the same thing slightly differently.
- **`argmax()`**: Returns the index of the largest value in an array. Used it to turn the model's 10-probability output into an actual digit prediction.
- **`try/except`**: Wrapped prediction and chart functions so one dodgy image or empty database doesn't crash the whole app.
- **`with` blocks (context managers)**: Used for `gr.Blocks`, `gr.Tab`, `gr.Row` -- everything indented inside the `with` belongs to that container. Same idea as `with open()` for files.
- **`yield` (generators)**: The training function yields progress text after each epoch so the UI can update live instead of freezing until it's done. Gradio detects the generator automatically.
- **List comprehensions**: `[row[0] for row in data]` to pull columns out of query results, much shorter than a loop.
- **`zip()`**: Pairs up two lists element-by-element. Used it to match accuracy and duration values when building hover text for the scatter chart.
- **`lambda`**: Mini inline functions -- used in `sorted(key=lambda x: ...)` to sort by confidence, and in `.click(fn=lambda img: ...)` to pass extra arguments to callbacks.
- **`sorted(key=)`**: Sort a list by a specific field. `sorted(model_rows, key=lambda m: m['confidence'], reverse=True)` puts highest-confidence model first.
- **`enumerate()`**: Gives you both the index and the value when looping. Used it to get `(digit, probability)` pairs from the model output.
- **`pd.DataFrame`**: A pandas table with named columns. Built it from a list of dictionaries where each dict is a row -- keys become column names. Gradio's `gr.Dataframe` component can display one directly. Used `pd.DataFrame(columns=[...])` for the empty state so column headers still show up.
- **`.then()` chaining**: Gradio's way of running multiple callbacks in sequence from one event. `button.click(fn=A).then(fn=B).then(fn=C)` runs A first, waits for it to finish, then B, then C. Each step can update different components. Used it for the refresh button that needs to update the history table, accuracy chart, and time chart one after another.
- **Multiple return values (tuples)**: Python lets you `return a, b, c` which packs the values into a tuple. Gradio unpacks them automatically into matching `outputs=` components in order. Used this for the predict function that returns original image, preprocessed image, and results all at once.
