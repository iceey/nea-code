...
The way you build a DataFrame was something I had to figure out. You create a list of dictionaries where each dict is one row -- keys are column names, like `[{'Run ID': 1, 'Arch': 'MLP'}, {'Run ID': 2, 'Arch': 'CNN'}]`. Then `pd.DataFrame(data)` turns the whole list into a proper table with rows and columns. I like this pattern because the column names travel with the data instead of being tracked separately -- with plain tuples or lists you'd have to remember which position means what, which gets confusing fast. For the empty state, `pd.DataFrame(columns=[...])` creates a table structure with just headers and no rows, which is what Gradio needs so the column labels still appear even when there's nothing to show yet.
