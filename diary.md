...
Added hover tooltips showing exact values for each point. Used `zip()` to pair up the accuracy and duration lists for building the hover text -- basically `zip(arch_accuracies, arch_durations)` gives you matched pairs without needing a counter variable. Plotly's hovertemplate is a bit fiddly (the formatting syntax is different from Python f-strings) but once it works it's really nice for exploring the data interactively.
