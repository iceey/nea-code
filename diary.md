...
The multiple return values thing is actually a proper Python feature I hadn't thought about much before. When you write `return a, b, c`, Python packs the three values into something called a tuple -- it's like a list but you can't change it after creating it. Normally when you call a function that returns multiple values, you'd unpack them with `x, y, z = my_function()` -- Python matches them up left to right. But in Gradio you don't unpack manually; instead you list the output components in `outputs=[comp1, comp2, comp3]` and Gradio automatically sends each return value to the matching component. So `return original, preprocessed, text` maps to `outputs=[original_display, preprocessed_display, prediction_text]` -- first value to first component, second to second, etc. For the error cases I return `None, None, "error message"` which keeps the image components blank but puts the message in the text box. Took me a minute to realise this is the same tuple concept just used differently.
