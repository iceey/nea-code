...
The `.then()` syntax ended up being really important to understand. In Gradio, `button.click(fn=A, outputs=out1)` runs function A when the button is clicked. But `.then(fn=B, outputs=out2)` chains another function that only runs after A has finished. You can keep going: `.click(fn=A).then(fn=B).then(fn=C)` means run A, wait for it to complete, then run B, wait again, then run C. Each one can update different components. So my refresh button does: load history table first, then create accuracy chart, then create time chart -- all from one button click, guaranteed in order. It's basically a sequential pipeline where each step waits for the previous one. I think this is similar to how promises work in JavaScript (we touched on that in class once) but I'm not 100% sure -- either way, the Gradio version is pretty intuitive once you see the pattern.
