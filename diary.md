...
The Small CNN database entry pointed at a model file that didn't exist (leftover from Phase 14's corruption issue). Had to patch the row. Also the initial output order was random because dictionaries don't guarantee order - fixed with sorting.
